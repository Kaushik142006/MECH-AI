...
def make_hex_nut_code(inner_r: float, outer_r: float, thickness: float, stl_path: str) -> str:
    return (
...
def make_circle_nut_code(inner_r: float, outer_r: float, thickness: float, stl_path: str) -> str:
    return (
        "from build123d import *\n"
        f"inner_r = {inner_r}\n"
        f"outer_r = {outer_r}\n"
...
- Completion — respond EXACTLY in this format:
  ...
  Summary:
  - Object: [Name]
  - Dimensions: Inner Dia=[X]mm, Outer Dia=[Y]mm, Thickness=[Z]mm
  - Shape: [Value]
...
def prompt_agent(summary: str) -> str:
    system = (
        ...
        "5. Define all dimensions as float variables.\n\n"
...
def coder_agent(blueprint: str, stl_path: str) -> str:
    ...
    system = (
        ...
        "with BuildPart() as b:\n"
        "    with BuildSketch(Plane.XY):\n"
        "        RegularPolygon(radius=outer_r, side_count=6)  # or Circle(radius=outer_r)\n"
        "    extrude(amount=thickness)\n"
        "    Hole(radius=inner_r)\n"
        f"export_stl(b.part, '{safe_path}')\n\n"
        "CRITICAL RULES:\n"
        "1. Export MUST be: export_stl(b.part, 'path')  — NOT b.part.export_stl()\n"
        "2. extrude() and Hole() MUST be indented inside `with BuildPart() as b:`\n"
...
def generate_fallback(summary: str, stl_path: str) -> str:
    ...
    if is_box and len(dims) >= 3:
        code = make_box_code(dims[0], dims[1], dims[2], safe_path)
    elif len(dims) >= 3:
        inner_r, outer_r, thickness = dims[0]/2, dims[1]/2, dims[2]
        code = make_hex_nut_code(inner_r, outer_r, thickness, safe_path) if is_hex \
               else make_circle_nut_code(inner_r, outer_r, thickness, safe_path)
    elif len(dims) >= 2:
        code = make_cylinder_code(dims[0]/2, dims[1], safe_path)
    else:
        code = make_hex_nut_code(6.0, 15.0, 10.0, safe_path)
